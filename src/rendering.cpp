...
// Check for build-time pre-baked SH coefficients.
// If prebaked_sh.inc exists (generated by tools/prebake.c during build),
// the SH coefficients are compiled in as constants — no runtime computation.
// If not, ProjectSkyboxToSH() computes them at boot (slow on ARM).
#if __has_include("prebaked_sh.inc")
#include "prebaked_sh.inc"
// shCoeffs[9][3] and shReady=true are defined in the .inc file
#define SH_PREBAKED 1
#else
static float shCoeffs[9][3] = {};  // 9 basis functions × RGB channels
static bool  shReady = false;
#define SH_PREBAKED 0
#endif
...
#if SH_PREBAKED
    TraceLog(LOG_INFO, "SH: Using pre-baked L2 coefficients (DC=%.3f,%.3f,%.3f)",
             shCoeffs[0][0], shCoeffs[0][1], shCoeffs[0][2]);
#else
    ProjectSkyboxToSH("skybox.png");
#endif

    // Try loading the pre-baked floor texture (generated at build time).
    // This eliminates the expensive per-pixel bake on device boot.
    Image bakedImg = LoadImage("baked_floor.png");
    if (bakedImg.data != NULL) {
        bakedFloorTex = LoadTextureFromImage(bakedImg);
        UnloadImage(bakedImg);
        TraceLog(LOG_INFO, "FLOOR: Loaded pre-baked floor texture");
        return;
    }

    // Fallback: bake at runtime (slow but works without prebake tool)
    TraceLog(LOG_WARNING, "FLOOR: baked_floor.png not found, baking at runtime");
...
// Prefers pre-baked tiles (skybox_tile_N.png) generated by tools/prebake.c;
// falls back to slicing skybox.png at runtime if tiles are absent.
void InitSkybox() {
    // Try pre-baked tiles first (already 256×256, no slicing needed)
    int loaded = 0;
    for (int t = 0; t < SKYBOX_TILES; t++) {
        char tileName[64];
        snprintf(tileName, sizeof(tileName), "skybox_tile_%d.png", t);
        Image tileImg = LoadImage(tileName);
        if (tileImg.data == NULL) break;
        skyboxTiles[t] = LoadTextureFromImage(tileImg);
        UnloadImage(tileImg);
        loaded++;
    }
    if (loaded == SKYBOX_TILES) {
        skyboxTileCount = SKYBOX_TILES;
        skyboxLoaded = true;
        TraceLog(LOG_INFO, "SKYBOX: Loaded %d pre-baked tiles (256x256 each)", SKYBOX_TILES);
        return;
    }
    // Clean up any partially loaded tiles
    for (int t = 0; t < loaded; t++) UnloadTexture(skyboxTiles[t]);

    // Fallback: slice the full panorama at runtime
    TraceLog(LOG_WARNING, "SKYBOX: Pre-baked tiles not found, slicing panorama at runtime");
